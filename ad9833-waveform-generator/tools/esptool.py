...
class BaseFirmwareImage(object):
    ...
    def add_segment(self, addr, data, pad_to=4):
        ...
        print 'AAAAAAAAAAAA addr=%x len=%x' % (addr, len(data))
        l = len(data)
        if l % pad_to:
            data += b"\x00" * (pad_to - l % pad_to)
            print 'padded: %x' % len(data)
        if l > 0:
...
class ELFFile(object):
    ...
    def load_section(self, section):
        ...
        try:
            ...
            print 'CMD %s' % ([tool_objcopy, "--only-section", section, "-Obinary", self.name, tmpsection])
...
class CesantaFlasher(object):
    ...
    def flash_write(self, addr, data, show_progress=False):
        assert addr % self._esp.ESP_FLASH_SECTOR == 0, 'Address must be sector-aligned: 0x%x' % addr
